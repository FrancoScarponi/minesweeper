...
function generateMines(rows, cols, count, seed) {
	var result = [];
	let randFn = SeededRandom(seed); //Si no se provee una seed se hace random
	while (result.length < count) {
		var r = Math.floor(randFn(seed) * rows);
		var c = Math.floor(randFn(seed) * cols);
		var key = r + "," + c;
